...
import { DepartementService, EntityArrayResponseType } from '../service/departement.service';
...
export class DepartementComponent implements OnInit {
  ...
  departements = signal<IDepartement[]>([]);
  isLoading = false;
  ...
  protected readonly departementService = inject(DepartementService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IDepartement): number => this.departementService.getDepartementIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.departements.set(dataFromBody);
  }
